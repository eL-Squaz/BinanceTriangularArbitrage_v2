...
const getPairs = async () => {
  const resp = await got('https://api.binance.us/api/v3/exchangeInfo');
  ...
  const validPairs = eInfo.symbols.filter(d => d.status === 'TRADING').map(d => d.symbol);
...
let ws = '';
const wsconnect = () => {
  ws = new Websocket(`wss://stream.binance.us:9443/ws`); //!ticker@arr
  ws.on('open', () => {
    ws.send(
      JSON.stringify({
        method: 'SUBSCRIBE',
...
module.exports = { getPairs, wsconnect, eventEmitter, processData };
